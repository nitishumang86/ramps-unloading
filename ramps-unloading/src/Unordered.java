	...
	*/	
	...
	public void Solve(){
		try {
		      ...
		      /* path variables (integral) */
		      GRBVar[] y = new GRBVar[numPaths];
		      int p=0;
		      for (int i = 0; i < pathList.size(); i++) {
		    	  for (int j = 0; j < pathList.get(i).size(); j++) {	
		    		  y[p] = model.addVar(0.0, GRB.INFINITY, 
		    				  pathList.get(i).get(j).pathLength, GRB.INTEGER, 
		    				  "pad"+String.valueOf(i)+" "+"path"+String.valueOf(j));	
		    		  p++;
		    	  }
		      }
		      
		      /* assignment variables (vehicle to location) */
		      GRBVar[][] x = new GRBVar[numVins][numLocations];
		      for (int i = 0; i < numVins; i++) {
		    	  for (int j = 0; j < numLocations; j++) {	
		    		  
		    		  x[i][j] = model.addVar(0.0, 1.0, 0.0, GRB.BINARY, 
		    				  				"assign_vehicle "+String.valueOf(i)+" "
		    				  				+" to location "+String.valueOf(j));	
		    	  }
		      }
		      
		      /* assignment variables (vehicle to path) */
		      GRBVar[][] z = new GRBVar[numVins][numPaths];
		      for (int i = 0; i < numVins; i++) {
		    	  for (p = 0; p < numPaths; p++) {	
		    		  
		    		  z[i][p] = model.addVar(0.0, 1.0, 0.0, GRB.BINARY, 
		    				  				"assign_vehicle "+String.valueOf(i)+" "
		    				  				+" to path "+String.valueOf(p));	
		    	  }
		      }
		      ...
		      /** Objective function - Set objective: minimize summation of d_p y_p*/
		     
		      GRBLinExpr expr = new GRBLinExpr();
		      p=0;
		      for (int i = 0; i < pathList.size(); i++) {
		    	  for (int j = 0; j < pathList.get(0).size(); j++) {	
		    		  expr.addTerm(pathList.get(i).get(j).pathLength, y[p]); 
		    		  p++;
		    	  }
		      }
		      ...
		      for(int l=0; l<numLocations; l++){
			      ...
			      for (int i = 0; i < pathList.size(); i++) {
			    	  for (int j = 0; j < pathList.get(i).size(); j++) {	
			    		  double constant = numVinHandlers*LocationPath(pathList.get(i).get(j), locationList.get(l));
			    		  expr.addTerm(constant, y[p]);
			    		  p++;
			    	  }
			      }
		    	  
			      for(int i=0; i<numVins; i++)
			    	  expr.addTerm(-1, x[i][l]);
		    	  
			      model.addConstr(expr, GRB.GREATER_EQUAL, 0,"linking_1_"+String.valueOf(l));
		      }
		      
		      /* linking constraints 2: x to y */
		      for(int l=0; l<numLocations; l++){
		    	  expr = new GRBLinExpr();
			      ...
			      for (int i = 0; i < pathList.size(); i++) {
			    	  for (int j = 0; j < pathList.get(i).size(); j++) {	
			    		  double constant = LocationPath(pathList.get(i).get(j), locationList.get(l));
			    		  expr.addTerm(constant, y[p]);
			    		  p++;
			    	  }
			      }
			      ...
			      model.addConstr(expr, GRB.LESS_EQUAL, 0,"linking_2_"+String.valueOf(l));
		      }
		      
		      /*  limit the minimum number of trips to Math.ceil(numVins/numVinHandlers)*/
		      /*expr = new GRBLinExpr();
			  p=0;
		      for (int i = 0; i < pathList.size(); i++) {
		    	  for (int j = 0; j < pathList.get(i).size(); j++) {	
	    	  ...
	    	  model.addConstr(expr, GRB.GREATER_EQUAL, Math.ceil((double)numVins/numVinHandlers),"max_trips");
	          */
		      
		      /* unique assignment of vehicle to path constraints */ 
		      for (int i = 0; i < numVins; i++) {
		    	  expr = new GRBLinExpr();
		    	  for (p = 0; p < numPaths; p++) 
		    		  expr.addTerm(1.0, z[i][p]);
		    	  model.addConstr(expr, GRB.EQUAL, 1,"uniqueAssignVehicletoPath"+String.valueOf(i));
		      }
		      
		      
		      /* linking constraints: y to z */
		      for (p = 0; p < numPaths; p++) {
		    	  expr = new GRBLinExpr();
		    	  expr.addTerm(numVinHandlers, y[p]);
		      
		    	  for (int i = 0; i < numVins; i++) 
		    		  expr.addTerm(-1.0, z[i][p]);
		    	  
		    	  model.addConstr(expr, GRB.GREATER_EQUAL, 0,"linking_y_z"+String.valueOf(p));
		      }
		      
		      
		      /* linking constraints: x to z */
		      p=0;
		      for (int i = 0; i < pathList.size(); i++) {
		    	  for (int j = 0; j < pathList.get(i).size(); j++) {	
		    		  ...
		    		  for(int m=0; m<numVins; m++){
		    			  ...
		    			  for(int l=0; l<numLocations; l++){
		    				  double constant = LocationPath(pathList.get(i).get(j), locationList.get(l));
				    		  expr.addTerm(constant, x[m][l]);
				    	  }
		    			  expr.addTerm(-1, z[m][p]);
		    		  
		    	        model.addConstr(expr, GRB.GREATER_EQUAL, 0,"linking_x_z"+String.valueOf(m*p));
		    		  }
		    		  p++;
		    	  }
		      }
		      ...
		      for (int i = 0; i < pathList.size(); i++) {
		    	  for (int j = 0; j < pathList.get(i).size(); j++) {	
		    		  if(pathList.get(i).get(j).locationSet.size()>2){
		    			  expr = new GRBLinExpr();
		      ...
		      /** Optimize */
		      model.getEnv().set(GRB.DoubleParam.TimeLimit, 300.0);
		      model.optimize();
		       
		      PrintSolution(model, y, x, z);
	...
	public void PrintSolution(GRBModel model, GRBVar[] y, GRBVar[][] x, GRBVar[][] z) throws GRBException {
		/** write output file */
		
	      try{
	    	  FileWriter fw = new FileWriter(outputfile);
	    	  
	    	  // print all y variable values
	    	  for(int p=0; p<numPaths; p++) {
	    		  if(y[p].get(GRB.DoubleAttr.X)>=1){
	    			  fw.write(y[p].get(GRB.StringAttr.VarName)+ " " +y[p].get(GRB.DoubleAttr.X));
	    		  	  fw.write("\n");
	    		  }
	    		  //System.out.println(y[p].get(GRB.StringAttr.VarName)+ " " +y[p].get(GRB.DoubleAttr.X));
	    	  }
	    	  ...
	    	  for(int i=0; i<numVins; i++) {
	    		  for(int l=0; l<numLocations; l++){
	    			  if(x[i][l].get(GRB.DoubleAttr.X)>=1){
	    				  fw.write(x[i][l].get(GRB.StringAttr.VarName)+ " " +
	    					  						x[i][l].get(GRB.DoubleAttr.X));
		    		  	  fw.write("\n");
	    			  }
	    		  }
	    	  ...
	    	  for(int i=0; i<numVins; i++) {
	    		  for(int p=0; p<numPaths; p++){
	    			  if(z[i][p].get(GRB.DoubleAttr.X)>=1){
	    				  fw.write(z[i][p].get(GRB.StringAttr.VarName)+ " " +
	    					  						z[i][p].get(GRB.DoubleAttr.X));
	    				  fw.write("\n");
